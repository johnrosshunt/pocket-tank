...
static int  s_rot;                               /* the frame's quarter turns: turned back into tank space */
...
void touch_port_poll(tank_t *t) {
    ...
    /* panel (px,py) -> tank (tx,ty), turned back by the frame's rotation
     * (rotate.h), so downstream gestures live in displayed space. The finger
     * bias can push a point off the glass: clamp it back to the edge. */
    float tx = s_lx, ty = s_ly;
    if (touched) rotate_panel_to_tank_f(s_rot, (float)pt[0].x, (float)pt[0].y, &tx, &ty);
    if (touched) {
        ty -= s_bias_y;
