...
/* the top row's place: in the panel for the PAGES keyboard, else along the open tank's top */
static void top_row(int *y, int *back_x, int *next_x) {
    bool panel_row = page_is_name() && s_kbd == SETUP_KBD_PAGES;
    *y      = panel_row ? SETUP_PANEL_TOP_BTN_Y  : SETUP_TOP_BTN_Y;
    *back_x = panel_row ? SETUP_PANEL_TOP_BACK_X : SETUP_TOP_BACK_X;
    *next_x = panel_row ? SETUP_PANEL_TOP_NEXT_X : SETUP_TOP_NEXT_X;
}
...
int setup_hit(float x, float y) {
    ...
    if (nav_on_top()) {
        int ty, bx, nx; top_row(&ty, &bx, &nx);
        if (in_box(x, y, nx, ty, SETUP_TOP_BTN_W, SETUP_BTN_H, m)) return SETUP_HIT_NEXT;
        if (!s_place && in_box(x, y, bx, ty, SETUP_TOP_BTN_W, SETUP_BTN_H, m)) return SETUP_HIT_BACK;
    } else {
...
static void nav(uint16_t *fb, int stride, bool top, const char *next_label, bool go) {
    int ty, tbx, tnx; top_row(&ty, &tbx, &tnx);
    int y = top ? ty : SETUP_BTN_Y, w = top ? SETUP_TOP_BTN_W : SETUP_BTN_W;
    render_button(fb, stride, top ? tbx : SETUP_BACK_X, y, w, SETUP_BTN_H, C_KEY, C_DIM, "BACK", 2);
    render_button(fb, stride, top ? tnx : SETUP_NEXT_X, y, w, SETUP_BTN_H,
...
void render_setup(const tank_t *t, uint16_t *fb, int stride, float clock) {
    ...
    if (s_page == SETUP_PG_WELCOME) {
        ...
        const int SL = 22, NX = grid ? SETUP_X + 160 : CX - FISH_NAME_MAX * SL / 2 + 2, NY = grid ? SETUP_Y + 58 : SETUP_PANEL_TOP_BTN_Y + (SETUP_BTN_H - 28) / 2;
        int n = name_len(f);
